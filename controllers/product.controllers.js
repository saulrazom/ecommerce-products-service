...
const createProduct = async (req, res) => {
    try {
        ...
        let imageUrl = null;
        if (req.file) {
            imageUrl = await uploadImageToS3(req.file.buffer, req.file.mimetype, req.file.originalname);
        }
        ...
        const newProduct = {
            ...
            imageUrl: imageUrl, // Added explicitly to match swagger update
        ...
        return res.status(500).json({ error: error.message || 'UnknownError' });
    }
};
...
const updateProduct = async (req, res) => {
    try {
        const { id } = req.params;
        const updates = req.body; // In multer, text fields are in req.body
        ...
        if (req.file) {
            const imageUrl = await uploadImageToS3(req.file.buffer, req.file.mimetype, req.file.originalname);
        ...
        for (const key of Object.keys(updates)) {
            if (allowedFields.includes(key) && updates[key] !== undefined) {
                ...
                // Handle type conversions from form-data
                if (key === 'price') {
                    expressionAttributeValues[valAlias] = Number(updates[key]);
                } else if (key === 'isActive') {
                    expressionAttributeValues[valAlias] = String(updates[key]) === 'true';
                } else {
